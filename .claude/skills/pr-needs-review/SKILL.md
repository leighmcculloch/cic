...
2. Extract all unique GitHub PR URLs from the messages. Normalize URLs (strip anchors, query params) to deduplicate.
...
```
<{url}|{owner}/{repo}#{number}> {title} `@{author}` `+{additions} -{deletions}`
```

Example:

```
<https://github.com/stellar/go/pull/1234|stellar/go#1234> Fix horizon ingestion bug `@janedoe` `+42 -7`
```
